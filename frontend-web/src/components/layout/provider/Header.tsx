...
import { Wallet, Bell, ChevronDown, Menu, Settings } from 'lucide-react';
...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 lg:left-72 right-0 h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center px-4 md:px-8 gap-4 md:gap-6 z-40 transition-all duration-300">
      
      {/* Lado Esquerdo: Perfil do Prestador (Igual ao Client) */}
      ...
      {/* Lado Direito: Rendimentos + Ações */}
          ...
          <div className="flex items-center gap-3 px-4 py-2 bg-accent/5 rounded-xl border border-accent/10 group hover:bg-accent/10 transition-all">
              ...
              <div className="flex flex-col -space-y-0.5">
                  <span className="text-sm font-black text-gray-900 tabular-nums">
                      Kz {monthlyEarnings.toLocaleString('pt-AO')}
                  </span>
              </div>
          </div>
